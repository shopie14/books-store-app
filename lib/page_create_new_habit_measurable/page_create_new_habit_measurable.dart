...
class PageCreateNewHabitMeasurable extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PageCreateNewHabitMeasurable> createState() =>
      _PageCreateNewHabitMeasurable();
  // State<Page_CreateNewHabitMeasurable> createState() => _Page_CreateNewHabitMeasurable();
}

class _PageCreateNewHabitMeasurable
    extends ConsumerState<PageCreateNewHabitMeasurable> {
  ...
  void initState() {
    ...
    if (widget.fHabit != null) {
      ...
      if (widget.fHabit?.getReminderTime() != null) {
        dateController.text =
            widget.fHabit?.getReminderTime() ?? "12:00"; //"12:00";
      ...
      } else if (widget.fHabit?.getFrequency() ==
          EHABITFREQUENCY.xTimesPerWeek) {
        frequencyValue = 'Every Week';
      } else if (widget.fHabit?.getFrequency() ==
          EHABITFREQUENCY.xTimesPerMonth) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          title: Text(
            widget.fHabit?.getTitle() ?? "New Habit",
          ...
          actions: <Widget>[
            Row(
              children: [
                InkWell(
                    child: Text("Save",
                        style: TextStyle(color: customColors.textColorLink)),
                    onTap: () {
                        ...
                        showDialog(
                            ...
                            builder: (context) {
                              return AlertDialog(
                                ...
                                title:
                                    const Text('Please enter a valid title.'),
                                actions: <Widget>[
                                  ElevatedButton(
                                    child: Text('Okay',
                                        style: TextStyle(
                                            color: customColors.textColorLink)),
                                    style: ButtonStyle(
                                        backgroundColor:
