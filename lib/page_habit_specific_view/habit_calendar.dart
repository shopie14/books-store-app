...
class _HabitCalendarState extends ConsumerState<HabitCalendar> {
  ...
  void loadHabitDates() {
    setState(() => {
          fhabitDates = widget.isarService
              .getHabitsDateCurrentMonth(_currentMonthNum, widget.habit.id)
        });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final calendarCarouselNoHeader = CalendarCarousel<Event>(
                          ...
                          ElevatedButton(
                            ...
                            style: ButtonStyle(
                                backgroundColor: MaterialStateColor.resolveWith(
                                    (states) => customColors.buttonNormal!),
                                foregroundColor: MaterialStateColor.resolveWith(
                                    (states) => customColors.textColor!),
                                surfaceTintColor:
                                    MaterialStateColor.resolveWith(
                                        (states) => Colors.transparent),
                                overlayColor: MaterialStateColor.resolveWith(
                                    (states) =>
                                        customColors.buttonNormalHover!),
      ...
      markedDateWidget: Positioned(
          child: Container(color: Colors.transparent, height: 4.0, width: 4.0),
          bottom: 4.0,
          left: 18.0),
    ...
    return SingleChildScrollView(
      ...
      children: <Widget>[
            ...
            children: <Widget>[
              ...
              TextButton(
                child: Icon(Remix.arrow_drop_left_line,
                    color: customColors.textColor, size: 48),
                onPressed: () {
                  setState(() {
                    ...
                    _currentMonth = DateFormat.yMMM().format(_targetDateTime);
                    _currentMonthNum =
                        DateFormat('y-M').format(_targetDateTime);
                  });
                  loadHabitDates();
                },
              ),
              TextButton(
                child: Icon(Remix.arrow_drop_right_line,
                    color: customColors.textColor, size: 48),
                onPressed: () {
                  setState(() {
                    _targetDateTime = DateTime(
        ...
        Container(
            color: customColors
                .background, // const Color.fromRGBO(31, 31, 31, 1.0),
                  ...
                  if (snapshot.connectionState == ConnectionState.done) {
                    ...
                    if (_modifyDate) {
                      ...
                      if (widget.habit.getType() == EHABITS.yesOrNo) {
                        var dateFound = false;
                        for (var h in snapshot.data!) {
                          if (DateFormat('y-M-dd').format(_selectedDate) ==
                              h.date) {
                            var i = h.id;
                            var hi = h.habitId;
                            var d = h.date;
                        ...
                        for (var h in snapshot.data!) {
                          if (DateFormat('y-M-dd').format(_selectedDate) ==
                            ...
                            if (v > 0) {
                              ...
                              widget.isarService.putHabitDate(
                                  HabitDate.fullWithId(id, hi, d, v));
                            } else {
                              _markedDateMap.removeAll(_selectedDate);
                            }
                        ...
                        if (!dateFound) {
                          ...
                          for (int i = 0;
                              i < int.parse(textController.text);
                              i++) {
                            _markedDateMap.add(
                                DateTime(_selectedDate.year,
                                    _selectedDate.month, _selectedDate.day),
